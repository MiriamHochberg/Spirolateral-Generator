...
def _buttonReleaseCallback(event):
    """
    Check if a button release event has occurred
    """
    global _clicks
    
    _makeSureWindowCreated()
    
    _clicks = [(event.x, event.y, "release")] + _clicks    
...
def mouseRelease():
    """
    Return True if the most recent mouse event was the release of a click 
    either left or right
    """
    global _mousePos
    if _mousePos:
        return _mousePos[2] == "release"
    raise Exception(
        "Can't determine mouse button if click events hasn't happened")
